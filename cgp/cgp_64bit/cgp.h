...
typedef struct
{
    ...
    // CGP parameters
    int inputs;  // number of circuit inputs
    int outputs; // number of circuit outputs
    int cols;    // number of columns in CGP
    int rows;    // number of rows in CGP
    int lback;   // number of levels back in CGP
    int nodeinputs; // number of inputs for each gate
    int nodeoutputs; // number of outputs for each gate
    int nodefuncs; // number of functions for each gate
...
void print_chrom(FILE *fout, chromosome p_chrom)
{
    ...
    {
        if (i % (params.nodeinputs + params.nodeoutputs) == 0)
            fprintf(fout, "([%d]", (i / (params.nodeinputs + params.nodeoutputs)) + params.inputs);
...
void init_paramsandluts()
{
    ...
    if (params.lback > params.cols)
        params.lback = params.cols;
    if (params.popsize > MAX_POPSIZE)
        params.popsize = MAX_POPSIZE;
    ...
    for (int i = 0; i < params.cols; i++)
    {
        ...
        if (minidx < params.inputs)
            minidx = params.inputs; // cgpnodes bloku zacinaji od params.inputs do params.inputs+m*n
