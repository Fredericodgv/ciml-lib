        ...
        if (i < params.geneoutidx)
        {
            ...
            { // mutate a gene that encodes connection
                if (col_values[col]->items > 1)
                {
                    do
                    {
                        rnd = col_values[col]->values[(rand() % (col_values[col]->items))];
                    } while (rnd == p_chrom[i]);
                    p_chrom[i] = rnd;
                }
            }
...
int main(int argc, char *argv[])
{
    ...
    dados = new nodetype[data_items];
    parsefile(params.datafname, dados, &params.inputs, &params.outputs);
    ...
    for (int generation = 0; generation < params.maxgenerations; generation++)
    {
        ...
        for (int i = 0; i < params.popsize; i++)
        {
            ...
            if ((i == fittest_idx) || (fit < bestfitval))
                continue;
